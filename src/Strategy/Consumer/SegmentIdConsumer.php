...
final readonly class SegmentIdConsumer implements StrategyInterface
{
    public function __construct(
        private readonly string $param,
    ) {
        if ($this->param === '') {
            throw new RuntimeException('SegmentIdConsumer requires a non-empty "param" parameter.');
        }
    }
    ...
    public function match(
        ...
        if ($segment === null || $segment === '' || !ctype_digit($segment)) {
            return false;
        }

        $routeParams[$this->param] = $segment;
        $path = $nextPath;
